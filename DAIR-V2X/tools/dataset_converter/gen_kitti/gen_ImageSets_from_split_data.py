...
def gen_ImageSet_inf_from_split_data(ImageSets_path, split_data_path, data_info_path, sensor_view="vehicle"):
    split_data = read_json(split_data_path)
    data_info_c = read_json(data_info_path)
    print(len(data_info_c))
    # import pdb
    # pdb.set_trace()
    veh2inf = {}
    ...
    # for i in range(len(split_data["train"])):
    #     name = split_data["train"][i]
    #     train_file = train_file + veh2inf[name] + "\n"

    # for i in range(len(split_data["val"])):
    #     name = split_data["val"][i]
    #     val_file = val_file + veh2inf[name] + "\n"

    # The test part of the dataset has not been released
    for i in range(len(split_data["test_A"])):
        name = split_data["test_A"][i]
        test_file = test_file + veh2inf[name] + "\n"
