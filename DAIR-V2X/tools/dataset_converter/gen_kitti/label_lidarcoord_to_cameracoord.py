...
# generate kitti label file for V2X-C dataset 
def gen_lidar2cam_c(source_root, target_root, cooperative_path, sensor_view):

    path_data_info = os.path.join(source_root, "data_info.json")
    data_info = read_json(path_data_info)
    frame2idx = {}
    for i, data in enumerate(data_info):
        frame_id = os.path.basename(data['pointcloud_path'])
        frame2idx[frame_id] = i

    # write_path = os.path.join(target_root, "label", label_type)
    # mkdir_p(write_path)
    path_data_c_info = os.path.join(cooperative_path, "data_info.json")
    data_c_info = read_json(path_data_c_info)

    # import pdb
    # pdb.set_trace()

    import tqdm
    for data_c in tqdm.tqdm(data_c_info):

        
        if sensor_view == 'cooperative':
            frame_id = os.path.basename(data_c["vehicle_pointcloud_path"])
        else:
            frame_id = os.path.basename(data_c["infrastructure_pointcloud_path"])
        if frame_id not in frame2idx.keys():
            print('data_info not found for', frame_id)
            continue
        data = data_info[frame2idx[frame_id]]
        if "calib_virtuallidar_to_camera_path" in data.keys():
            calib_lidar2cam_path = data["calib_virtuallidar_to_camera_path"]
        else:
            calib_lidar2cam_path = data["calib_lidar_to_camera_path"]
        ...
        cooperative_label_path = data_c['cooperative_label_path']
        labels_name = os.path.basename(cooperative_label_path) #data["label_" + label_type + "_std_path"]
        labels = read_json(os.path.join('./data/DAIR-V2X/cooperative-vehicle-infrastructure/cooperative_i/label_new', labels_name))
        ...
        #labels_path = labels_path.replace("virtuallidar", "lidar")
        write_path = os.path.join(target_root, labels_name)

        with open(write_path, "w") as f:
            json.dump(labels, f)


def gen_lidar2cam_ci(source_root, target_root, cooperative_path, label_type="lidar"):

    path_data_info = os.path.join(source_root, "data_info.json")
    data_info = read_json(path_data_info)
    frame2idx = {}
    for i, data in enumerate(data_info):
        frame_id = os.path.basename(data['label_camera_std_path'])
        frame2idx[frame_id] = i
    ...
    for data_c in tqdm.tqdm(data_c_info):
        ...
        cooperative_label_path = data_c['cooperative_label_path']
        frame_id = os.path.basename(cooperative_label_path)
        ...
        labels_name = os.path.basename(cooperative_label_path) #data["label_" + label_type + "_std_path"]
        labels = read_json(os.path.join('./data/DAIR-V2X/cooperative-vehicle-infrastructure/cooperative/label_new', labels_name))
