...
class VICDataset(DAIRV2XDataset):
    def __init__(self, path, args, split="train", sensortype="lidar", extended_range=None):
        ...
        for elem in frame_pairs:
            ...
            if osp.isfile(osp.join(path, elem["cooperative_label_path"])):
            ...
            #for test the upperbound of latefusion with gt of inf_side
            test_inf_gt = 0
            if test_inf_gt:
                label_i = Label(osp.join(path, "infrastructure-side", inf_frame["label_lidar_std_path"]), filt)
                label_v["label_i"] = label_i
    ...
    def get_split(self, split_path, split, frame_pairs):
        ...
        for frame_pair in frame_pairs:
            ...
            if veh_frame_idx in split_data and veh_frame_idx not in visit_map:
                visit_map.append(veh_frame_idx)
